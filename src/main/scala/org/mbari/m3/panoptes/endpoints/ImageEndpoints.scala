/*
 * Copyright 2017 Monterey Bay Aquarium Research Institute
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package org.mbari.m3.panoptes.endpoints

import scala.concurrent.ExecutionContext
import sttp.tapir.Endpoint
import sttp.model.StatusCode
import sttp.tapir.*
import sttp.tapir.Endpoint
import sttp.tapir.generic.auto.*
import sttp.tapir.json.circe.*
import sttp.tapir.server.ServerEndpoint
import java.io.File
import sttp.model.Part
import org.mbari.m3.panoptes.etc.jwt.JwtService
import org.mbari.m3.panoptes.etc.circe.CirceCodecs.Implicits.given
import org.mbari.m3.panoptes.domain.{BadRequest, ErrorMsg, HealthStatus, ImageParams, NotFound, ServerError, StatusMsg, Unauthorized}
import sttp.tapir.EndpointIO.annotations.body
import scala.concurrent.Future

class ImageEndpoints(jwtService: JwtService)(using ec: ExecutionContext) extends Endpoints {

    private val log = System.getLogger(getClass().getName())
    given givenJwtService: JwtService = jwtService

    case class ImageUpload(file: Part[File])

    val uploadImage =
        secureEndpoint
            .post
            .in("v1" / "image" / path[String]("camera_id") / path[String]("deployment_id") / path[String]("name"))
            .in(multipartBody[ImageUpload])
            .out(jsonBody[ImageParams])
            .errorOut(
                oneOf[ErrorMsg](
                    oneOfVariant(statusCode(StatusCode.BadRequest).and(jsonBody[BadRequest])),
                    oneOfVariant(statusCode(StatusCode.NotFound).and(jsonBody[NotFound])),
                    oneOfVariant(statusCode(StatusCode.InternalServerError).and(jsonBody[ServerError])),
                    oneOfVariant(statusCode(StatusCode.Unauthorized).and(jsonBody[Unauthorized]))
                )
            )
            .name("uploadImage")
            .description("Upload an image")
            .tag("image")

    
    val uploadImageImpl =
        uploadImage
            .serverSecurityLogic(jwtOpt => verify(jwtOpt))
            .serverLogic(_ => imageUpload =>
                val file = imageUpload.file
                val cameraId = file.filename
                val deploymentId = file.filename
                val name = file.filename
                Future(Right(ImageParams(cameraId, deploymentId, name))
            )

    
    val resolveImageLocation =
        openEndpoint
            .get
            .in("v1" / "image" / path[String]("camera_id") / path[String]("deployment_id") / path[String]("name"))
            .out(jsonBody[ImageParams])
            .errorOut(
                oneOf[ErrorMsg](
                    oneOfVariant(statusCode(StatusCode.BadRequest).and(jsonBody[BadRequest])),
                    oneOfVariant(statusCode(StatusCode.InternalServerError).and(jsonBody[ServerError]))
                )
            )

    val downloadImage =
        openEndpoint
            .get
            .in("v1" / "image" / "download" / path[String]("camera_id") / path[String]("deployment_id") / path[String]("name") )
            .out(byteArrayBody)
            .errorOut(
                oneOf[ErrorMsg](
                    oneOfVariant(statusCode(StatusCode.BadRequest).and(jsonBody[BadRequest])),
                    oneOfVariant(statusCode(StatusCode.NotFound).and(jsonBody[NotFound])),
                    oneOfVariant(statusCode(StatusCode.InternalServerError).and(jsonBody[ServerError]))
                )
            )
  
}
